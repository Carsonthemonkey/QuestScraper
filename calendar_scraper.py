...
def get_relevant_event_cards(soup, start_day: date, day_num: int):
    ...
    for event_date, card_group in zip(dates, card_groups):
        ...
        cards.extend(
            [card.contents[3] for card in full_cards]
        )  # Why is it 3 again? seems unusual TODO: make this more readable and robust
    ...
    try:
        next_page_link = soup.find(
            attrs={"aria-label": "Next page"}, class_="em-pagination-item"
        ).attrs["href"]
    except AttributeError:
        warnings.warn(
            "Could not find next page of events. May be missing", RuntimeWarning
        )
        return cards
...
def scrape_events(save_path: str, start_day: date, day_num: int, max_words: int):
    cards = get_relevant_event_cards(
        get_page_source(EVENT_CALENDAR_URL), start_day, day_num
    )
    ...
    with open(
        f"{save_path}/{str(date.today())}-events-readable.md", "w", encoding="utf-8"
    ) as f:
        prev_date = None
        written_descriptions = []
        for event in event_data:
            f.write(
                f"# {date_parse(event['date']).strftime('%A, %B %d, %Y')}"
...
def parse_blotter(soup):
    ...
    for i in range(0, len(paragraphs), 2):
        ...
        try:
            ...
            case["date"] = (
                str(date_parse(case_date))[:11] + time
            ...
            warnings.warn(
                f"Missing case data for case. Data collected:\n {case}\n Error: {e}",
                RuntimeWarning,
            )
        cases.append(case)
...
def scrape_blotter(save_path: str, max_words):
    ...
    with open(
        f"{save_path}/{str(date.today())}-blotter-readable.md", "w", encoding="utf-8"
    ) as f:
        ...
        for case in cases:
            ...
            # Incorrect date format
                ...
                case_datetime = None
                readable_date = case["date"]
            f.write(f"## {readable_date} \n\n")
            time_string = case_datetime.strftime("%H:%M") if case_datetime else f"{case["date"][-4:-2]}:{case["date"][-2:]}"
            f.write(
                f"**{case['case_number']}; {case['description']}; {case['location']}; {time_string};**\n\n"
            )
